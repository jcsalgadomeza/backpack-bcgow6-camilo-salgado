...
import (
	...
	"github.com/jcsalgadomeza/backpack-bcgow6-camilo-salgado/GO-WEB/C4-TM/domains"
)
...
func (r *repository) Update(id int, nombre, tipo string, cantidad int, precio float64) (domains.Product, error) {
	...
	if !updated {
		return domains.Product{}, fmt.Errorf("products id %d not exists", id)
	}
	return p, nil
}
...
func (r *repository) UpdateName(id int, nombre string) (domains.Product, error) {
	...
	if !updated {
		return domains.Product{}, fmt.Errorf("product id %d not exitst", id)
	}
